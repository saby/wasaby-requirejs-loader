...
function getExportNames(library): string[] {
    if (typeof library !== 'object') {
        return library;
    }

    return Object.keys(library as object).filter((exp) => {
        return !exp.includes('/');
    });
}
...
export function extract<T>(exports: T | ILibrary<T>, info: IParsed): T | ReferenceError {
    ...
    if (info.path.length) {
        ...
        for (let i = 0; i < info.path.length; i++) {
            ...
            const availableExportMsg = `Available export names: ${getExportNames(module)}`;
