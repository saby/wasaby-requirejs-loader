...
import { extract, ILibrary, parse } from './Library';
...
export async function loadAsync<T>(
    ...
    if ((requirejs as IRequireExt)?.isNewRequire) {
        try {
            //@ts-ignore
            let result = (await requirejs([name]))[0];

            if (
                !name.includes(':') &&
                result &&
                (result as ILibrary<T>).__esModule &&
                (result as ILibrary<T>).default
            ) {
                result = (result as ILibrary<T>).default as T;
            }
...
export function loadSync<T>(name: string): T {
    if ((requirejs as IRequireExt)?.isNewRequire) {
        ...
        if (
            ...
            return (result as ILibrary<T>).default as T;
        }
