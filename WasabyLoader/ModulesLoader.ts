...
const DEFAULT_WARMUP_TIMEOUT_MS = 2000;
...
export function onModuleLoaded(callback: ModuleLoadCallback) {
    if ((requirejs as IRequireExt)?.isNewRequire) {
        //@ts-ignore
        requirejs.instance.onLoadModule(callback);
    } else {
...
export function offModuleLoaded(callback: ModuleLoadCallback) {
    if ((requirejs as IRequireExt)?.isNewRequire) {
        //@ts-ignore
        requirejs.instance.offLoadModule(callback);
    } else if (modulesManager) {
        getModulesManager().offModuleLoaded(callback);
    }
}
...
function getWarmupTimeout(): number {
    if (typeof document === 'object') {
        const matches = String(document.cookie).match(/s3WarmupTimeout=([^;]+)[;]?/);
    ...
    return DEFAULT_WARMUP_TIMEOUT_MS;
}
...
export async function loadAsync<T>(
    ...
    // не надо запускать проверку на наличие запрашиваемого модуля в contents, если
    // запрашивается optional зависимость. Подразумевается, что запрашиваемой зависимости
    // может не быть в данном приложении
    if (!name.startsWith('optional!') && !ModulesManager.isModule(name)) {
    ...
    if ((requirejs as IRequireExt)?.isNewRequire) {
        try {
            let result;
            const normName = name.includes(':') ? name : `${name}:default`;

            if (isServerSide) {
                result = requirejs(normName);
    ...
    try {
        ...
        if (err instanceof Error) {
            // Пришлось спилить определения url из-за двух проблем, которые не дают корректно определить url:
            // 1. Если модуль грузиться через плагины, то require не связывает имя с url.
            // 2. Если модуль входит в пакет, мы грузим его через наши прослойки и require ничего не знает об этом
            // и привязывает модуль к url до оригинально файла, а не до пакета.
            // В новом загрузчике вся логика будет внутри поэтому url будет определяться корректно.
...
export function loadSync<T>(name: string): T {
    if ((requirejs as IRequireExt)?.isNewRequire) {
        //@ts-ignore
        const result = requirejs(name.includes(':') ? name : `${name}:default`);
