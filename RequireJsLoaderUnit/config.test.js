define("RequireJsLoaderUnit/config.test", ["require", "exports", "chai", "RequireJsLoader/config"], function (require, exports, chai_1, config_1) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var global = this || (0, eval)('this'); // eslint-disable-line no-eval
    describe('RequireJsLoader/config', function () {
        var contents = global.contents;
        var wsConfig = global.wsConfig;
        beforeEach(function () {
            global.contents = {};
            global.wsConfig = Object.assign({}, global.wsConfig);
        });
        afterEach(function () {
            global.contents = contents;
            global.wsConfig = wsConfig;
        });
        context('when affects requirejs()\'s behaviour', function () {
            it('shouldn\'t throw ReferenceError for file in resources folder', function () {
                global.wsConfig.resourceRoot = '/assets/';
                return new Promise(function (resolve) {
                    requirejs(['/assets/contents.js'], resolve, function (err) {
                        chai_1.assert.notInstanceOf(err, ReferenceError);
                        resolve();
                    });
                });
            });
        });
        context('when affects require.defined()\'s behaviour', function () {
            it('shouldn\'t throw ReferenceError if module doesn\'t exist', function () {
                global.wsConfig.resourceRoot = '/assets/';
                chai_1.assert.isFalse(requirejs.defined('path/to/resource'));
            });
        });
        context('patchContext()', function () {
            var defContext = requirejs.s.contexts._;
            var restoreContext;
            beforeEach(function () {
                restoreContext = config_1.patchContext(defContext, {});
            });
            afterEach(function () {
                if (restoreContext) {
                    restoreContext();
                }
            });
            context('when affects context.nameToUrl()\'s behaviour', function () {
                it('shouldn\'t add .js extension if url already ends with .wml or .tmpl', function () {
                    chai_1.assert.isTrue(defContext.nameToUrl('foo/bar.wml').endsWith('/foo/bar.wml'));
                    chai_1.assert.isTrue(defContext.nameToUrl('foo/bar.tmpl').endsWith('/foo/bar.tmpl'));
                });
            });
        });
    });
});
