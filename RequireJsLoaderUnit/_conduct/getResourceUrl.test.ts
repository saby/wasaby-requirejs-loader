...
describe('RequireJsLoader/_conduct/getResourceUrl', () => {
    ...
    test('should insert rtl postfix for css only if direction is equal to rtl', () => {
        ...
        expect(getResourceUrl('/path/to/resource.js', undefined, true, 'rtl')).toStrictEqual(
            '/path/to/resource.js'
        );

        expect(getResourceUrl('/cdn/path/to/resource.css', undefined, true, 'rtl')).toStrictEqual(
            '/cdn/path/to/resource.css'
        );

        expect(getResourceUrl('/path/to/resource.css', undefined, true, 'rtl')).toStrictEqual(
            '/path/to/resource.rtl.css'
        );

        expect(getResourceUrl('/ThemesModule/resource.css', undefined, true, 'rtl')).toStrictEqual(
            '/ThemesModule/resource.rtl.css'
        );
    });
    ...
    test("shouldn't add domain name for svg files and contents meta", () => {
        ...
        expect(getResourceUrl('/assets/router.js')).toStrictEqual('/assets/router.js');

        // нужно также проверить, что ресурс с таким же именем, что и корневой, но лежащий внутри модуля, также получает
        // cdn-домен
        expect(getResourceUrl('/assets/a/router.js')).toStrictEqual('//foo.bar/assets/a/router.js');
    });

    test('should add shard domain name for non cdn urls and empty resourceroot', () => {
        wsConfig.resourceRoot = '/';
        wsConfig.staticDomains = { domains: ['foo.bar'] };
        wsConfig.shardDomain = 'shard.foo.bar';

        expect(getResourceUrl('/a/resource.js')).toStrictEqual('//foo.bar/a/resource.js');

        expect(getResourceUrl('/contents.min.js')).toStrictEqual('//shard.foo.bar/contents.min.js');

        expect(getResourceUrl('/contents.js')).toStrictEqual('//shard.foo.bar/contents.js');

        expect(getResourceUrl('/router.js')).toStrictEqual('//shard.foo.bar/router.js');

        expect(getResourceUrl('/a/router.js')).toStrictEqual('//shard.foo.bar/a/router.js');
    });

    test('should add shard domain name for non cdn urls and non-empty resourceroot', () => {
        wsConfig.resourceRoot = '/assets/';
        wsConfig.staticDomains = { domains: ['foo.bar'] };
        wsConfig.shardDomain = 'shard.foo.bar';
        ...
        expect(getResourceUrl('/assets/contents.min.js')).toStrictEqual(
            '//shard.foo.bar/assets/contents.min.js'
        );

        expect(getResourceUrl('/assets/contents.js')).toStrictEqual(
            '//shard.foo.bar/assets/contents.js'
        );

        expect(getResourceUrl('/assets/router.js')).toStrictEqual(
            '//shard.foo.bar/assets/router.js'
        );
