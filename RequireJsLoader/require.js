...
(function (global, setTimeout) {
    ...
    * @param {Error} [originalError] the original error, if there is one.
    *
    * @returns {Error}
    */
   function makeError(id, message, originalError, requireModules, fileContent) {
      ...
      if (originalError) {
         if (originalError.message || originalError.stack) {
            error.message +=
               '\nOriginalError: ' + originalError.message + '\n' + originalError.stack;

            // В require.js мы можем столкнуться со следующей ошибкой и её цепочкой дочерних ошибок
            // err = {
            //   message: 'Что то произошло в скрипте _@r50',
            //   stack: 'стек require без ссылки на дочернюю ошибку',
            //   originalError: {
            //      message: 'произошла ошибка в скрипте MyModule/myFile',
            //      stack: 'стек оригинальной ошибки'
            //   }
            // }
            // и в случае анонимного скрипта(например _@r50) сообщение дочерней оригинальной ошибки можно увидеть только
            // в стеке дочерней ошибки, соответственно нам надо также проксировать в стек родительской ошибки стек
            // дочерней ошибки, иначе выводится только ошибка "Что то произошло в скрипте _@r50" и данный стек и это
            // не даёт никакой информации о том, что на самом деле сломалось
                  ...
                  if (isFunction(factory)) {
                     ...
                     if (
                        ...
                        try {
                           ...
                           err = makeError(
                              null,
                              'Error executing callback function from module "' + id + '"',
                              e,
                              [],
                              factory.toString()
                           );
         ...
         callPlugin: function () {
            ...
            on(
               ...
               bind(this, function (plugin) {
                  ...
                  //HACK: fromText can define module without plugin as undefined. Cancel that behavior.
                  load.fromTextFixed = bind(this, function (text) {
                     /*jslint evil: true */
      ...
      context = {
         ...
         nameToUrl: function (moduleName, ext, skipExt) {
            ...
            if (req.jsExtRegExp.test(moduleName)) {
               ...
               url =
                  (url.charAt(0) === '/' || url.match(/^[\w\+\.\-]+:/) ? '' : config.baseUrl) + url;
            }
