...
define('text', ['module'], function (module) {
   ...
   var buildMap = {};
   var masterConfig = (typeof module.config === 'function' && module.config()) || {};
   ...
   if (
      ...
      fs = require.nodeRequire ? require.nodeRequire('fs') : require('fs');

      text.get = function (url, callback, errback) {
         try {
            if (isRemote.test(url)) {
               var nodeReq = (process.domain && process.domain.req && process.domain.req) || {},
                  headers = nodeReq.headers || {},
                  proto = headers['x-forwarded-proto'] || headers['x-forwarded-protocol'] || 'http';
      ...
      // все что выше нам не подошло, ну видимо мы на сервере
      text.get = function (url, callback) {
         var output = TextRequest(url);
         callback(output);
      };
   }
   return text;
