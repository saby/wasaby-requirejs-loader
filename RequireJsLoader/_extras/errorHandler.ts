import {
    ...
    IRequireModule,
    IRequireExt,
    IRequireModuleHolder,
} from '../requireTypes';
import { undefineAncestors, undefineModule } from 'RequireJsLoader/conduct';
...
function undefineFailedChains(context: IRequireContext, logger: ILogger): void {
    ...
    if (hasError) {
        registryNames.forEach((moduleName) => {
            const module = registry[moduleName];
            undefineModule(context.require, moduleName, logger, module?.error?.toString());
        });
        context.require(
