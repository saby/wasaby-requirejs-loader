...
function getDependencyList(
    ...
    for (const moduleName of modules) {
        ...
        if (deps) {
            getDependencyList(deps, ignoreList, list, isServerModule);
        }
    }
...
function getResourceBlockNumber(): number {
    ...
    const previous = request.previousResourceBlockNumber || 0;
    const current = previous + 1;

    request.previousResourceBlockNumber = current;
...
export default function getResourcesList(
    ...
    if (resourceBlockNumber === 1) {
        ...
        modules.push(loadingsHistoryI18n.regions[controller.currentCountry]);
