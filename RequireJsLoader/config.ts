...
// Полиффил globalThis для IE11. https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/globalThis
// Данное свойство нужно инициализировать как можно раньше до исполнения все скриптов,
// а SbisUI/polyfill грузиться через require и поэтому это поздно.
// Поэтому вставляем полифил сюда в конфиг require, который грузиться и исполняется первым.
if (typeof globalThis === 'undefined') {
    ...
    globalEnv.globalThis = globalEnv;
}
...
define(
    ...
    (() => {
        ...
        function getWsConfig(): IWsConfig {
            ...
            return GLOBAL.wsConfig || (GLOBAL.wsConfig = {} as IWsConfig);
        }

        function getContents(): IContents | undefined {
            return (globalThis as unknown as IPatchedGlobal).contents;
        }
                        ...
                        if (typeof exports === 'function') {
                            ...
                            if (proto) {
                                ...
                                if (!(exports as ITensorFunction).hasOwnProperty('_moduleName')) {
                                    ...
                                    arrowFunction._isPrivateModule = isPrivateModule || undefined;
                                }
                            }
                        } else if (
        ...
        const debug: IDebug = {
            ...
            RESOURCE_ROOT: getWsConfig().resourceRoot,
            CDN_ROOT: getWsConfig().cdnRoot,
            MODULES: [],
                ...
                if (url) {
                    return (
                        ...
                        this.MODULES.some((mod) => {
                            if (
                                url.indexOf(this.RESOURCE_ROOT + mod) !== -1 ||
                                url.indexOf(this.CDN_ROOT + mod) !== -1
                            ) {
                                return true;
                            }
         ...
         */
        function postProcessBundles(bundles: IPatchedGlobal['bundles']): IPatchedGlobal['bundles'] {
         ...
         */
        function buildHandlers(config: IWsConfig): IHandlersInternal {
                ...
                if (moduleInfo.hasOwnProperty('hasBundles')) {
                    ...
                    requirejs(
                        ...
                        (currentContents: IContents) => {
                            if (
                                currentContents?.modules?.[moduleName].hasOwnProperty('hasBundles')
                            ) {
                                requirejs(
                                    [`${moduleName}/packageMap.json`],
                                    (bundles: Record<string, string>) => {
        ...
        function createConfig(
            ...
            const config: RequireConfig = {
                ...
                paths: {
                    // React
                    react: pathJoin(reactRoot, 'v17/react/react'),
                    'react/jsx-dev-runtime': pathJoin(
                        reactRoot,
                        'v17/react/jsx-dev-runtime/react-jsx-dev-runtime'
                    ),
                    'react/jsx-runtime': pathJoin(
                        reactRoot,
                        'v17/react/jsx-runtime/react-jsx-runtime'
                    ),
                    'react/react-server': pathJoin(reactRoot, 'v19/react/react-server'),
                    'react-compiler-runtime': pathJoin(
                        reactRoot,
                        'v19/react/react-compiler-runtime'
                    ),

                    // React DOM
                    'react-dom': pathJoin(reactRoot, 'v17/react-dom/react-dom'),
                    'react-dom/client': pathJoin(
                        reactRoot,
                        'v17/react-dom/client/react-dom-client'
                    ),
                    'react-dom/server': pathJoin(
                        reactRoot,
                        'v17/react-dom/server/react-dom-server-legacy.browser'
                    ),
                    'react-dom/test-utils': pathJoin(
                        reactRoot,
                        'v17/react-dom/test-utils/react-dom-test-utils'
                    ),
                    'react-dom/testing': pathJoin(
                        reactRoot,
                        'v17/react-dom/testing/react-dom-testing'
                    ),
                    'react-dom/profiling': pathJoin(reactRoot, 'v19/react-dom/react-dom-profiling'),
                    ...
                    'react-is': pathJoin(reactRoot, 'v19/react-is/react-is'),
                    'react-cache': pathJoin(reactRoot, 'v19/react-cache/react-cache'),
                    ...
                    'react-refresh/runtime': pathJoin(
                        reactRoot,
                        'v19/react-refresh/react-refresh-runtime'
                    ),
                    'react-server': pathJoin(reactRoot, 'v19/react-server/react-server'),

                    // Scheduler
                    'scheduler-react': pathJoin(reactRoot, 'v19/scheduler/scheduler'),
                    'scheduler-react/unstable_mock': pathJoin(
                        reactRoot,
                        'v19/scheduler/scheduler-unstable_mock'
                    ),
                    'scheduler-react/unstable_post_task': pathJoin(
                        reactRoot,
                        'v19/scheduler/scheduler-unstable_post_task'
                    ),
                    'scheduler-react/native': pathJoin(reactRoot, 'v19/scheduler/scheduler.native'),

                    // use-subscription, use-sync-external-store
                    'use-subscription': pathJoin(
                        reactRoot,
                        'v19/use-subscription/use-subscription'
                    ),
                    'use-sync-external-store': pathJoin(
                        reactRoot,
                        'v19/use-sync-external-store/use-sync-external-store'
                    ),
                    'use-sync-external-store/shim': pathJoin(
                        reactRoot,
                        'v19/use-sync-external-store/use-sync-external-store-shim'
                    ),
                    'use-sync-external-store/shim/with-selector': pathJoin(
                        reactRoot,
                        'v19/use-sync-external-store/use-sync-external-store-shim-with-selector'
                    ),
                    'use-sync-external-store/with-selector': pathJoin(
                        reactRoot,
                        'v19/use-sync-external-store/use-sync-external-store-with-selector'
                    ),
                    'use-sync-external-store/shim/index.native': pathJoin(
                        reactRoot,
                        'v19/use-sync-external-store/use-sync-external-store-shim.native'
                    ),
        ...
        function prepareEnvironment(require: IRequireExt, withHandlers: IHandlersInternal): void {
...
declare module 'RequireJsLoader/config' {
    ...
    export const RELEASE_MODE: BuildMode;
    ...
    export function applyConfig(require: Require, wsConfig: IWsConfig, context?: string): Require;

    export function prepareEnvironment(require: IRequireExt, withHandlers: IHandlersInternal): void;
