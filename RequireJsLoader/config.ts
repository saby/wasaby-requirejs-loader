    ...
    (() => {
        ...
        function isSuffixNeeded(url: string | undefined) {
            if (typeof url === 'string') {
                ...
                // для .css.json.js нужны на СП именно релизные файлы, чтобы сгенерированная верстка
                // на сервере и на клиенте не отличалась для минифицированных через css modules классов
                // это нужно для избежания ошибок реакта react-dom error! hydration error!
                if (url.indexOf('.css.json.js') !== -1) {
                    return true;
                }
            }
        ...
        function createConfig(
            ...
            const config: RequireConfig = {
                ...
                paths: {
                    ...
                    'react-cache': pathJoin(reactRoot, `v${REACT_VERSION}/react-cache/react-cache`),
                    ...
                    // Scheduler
                    'scheduler-react': pathJoin(reactRoot, `v${REACT_VERSION}/scheduler/scheduler`),
        ...
        function applyConfig(require: Require, wsConfig: IWsConfig, context?: string): Require {
            ...
            if (globalEnv.bundles && contents && BUILD_MODE === RELEASE_MODE && !wsConfig.pagexPackages) {
