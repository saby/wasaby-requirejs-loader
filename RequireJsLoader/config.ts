...
define(
    ...
    (() => {
        ...
        function getModuleName(name: string): string {
            ...
            if (name.indexOf('react') === 0  || name.includes('use-sync-external-store')) {
                return 'React';
            }
        ...
        if (!globalEnv.requirejs || globalEnv.requirejs.isNewRequire) {
            ...
            const REACT_VERSION: ReactVersion = GLOBAL.contents?.modules?.React?.version || 19;
            ...
            const createConfig = (
                ...
                const config: RequireConfig = {
                    ...
                    paths: {
                        ...
                        // React Reconciler
                        'react-reconciler': pathJoin(
                            reactRoot,
                            `v${REACT_VERSION}/react-reconciler/react-reconciler`
                        ),
                        ...
                        // Scheduler
                        scheduler: pathJoin(reactRoot, `v${REACT_VERSION}/scheduler/scheduler`),
                        'scheduler/unstable_mock': pathJoin(
                            reactRoot,
                            `v${REACT_VERSION}/scheduler/scheduler-unstable_mock`
                        ),
                        'scheduler/unstable_post_task': pathJoin(
                            reactRoot,
                            `v${REACT_VERSION}/scheduler/scheduler-unstable_post_task`
                        ),
                        'scheduler/native': pathJoin(reactRoot, 'v19/scheduler/scheduler.native'),
                        ...
                        pixi8: `${cdnRoot}PixiJS/8.18.1-p2/pixi${
                            debug.isDebuggingModule(cdnRoot + 'PixiJS/') ? '' : '.min'
                        }`,
                        'pixi-react7': `${cdnRoot}PixiReact/7.1.3-p1/pixi-react${
                            debug.isDebuggingModule(cdnRoot + 'PixiReact/') ? '' : '.min'
                        }`,
                        'pixi-react8': `${cdnRoot}PixiReact/8.0.4-p2/pixi-react${
                            debug.isDebuggingModule(cdnRoot + 'PixiReact/') ? '' : '.min'
                        }`,
