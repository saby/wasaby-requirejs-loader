...
define('RequireJsLoader/config', (() => {
    ...
    function buildHandlers(config: RequireJsLoader.IWsConfig): IHandlersInternal {
        ...
        const NON_CDN_URLS_MATCH = /(contents|router)(\.min)?\.js/;
        ...
        function isExcludedUrl(url: string) {
            ...
            return url.indexOf('.svg') !== -1 || NON_CDN_URLS_MATCH.test(url) || url.indexOf('manifest.json') !== -1;
        }
        ...
        function checkUrlForDebugMode(url: string, debugCookieValue: string, checkReactMode?: boolean): boolean {
