...
(function () {
    ...
    var js = (defineName, filePath, context) => {
        ...
        try {
            return load$1(defineName, path, context);
        }
        catch (err) {
     ...
     */
    function wml (defineName, filePath, context) {
    ...
    class ServerRequire extends BaseRequire {
        ...
        getDebugModules() {
            // @ts-ignore
            const cacheDebugModules = process?.domain?.req?.debugModules;
            if (cacheDebugModules) {
                return cacheDebugModules;
            }
            // @ts-ignore
            const debugModules = this.buildDebugModules(process?.domain?.req?.cookies?.s3debug);
            // @ts-ignore
            if (process?.domain?.req) {
                // @ts-ignore
                process.domain.req.debugModules = debugModules;
            }
            return debugModules;
        }
        getNumberStaticDomain() {
            // @ts-ignore
            return process?.domain?.req?.cookies?.res_loader_cdn_idx || 0;
        }
        enableRtlDirection() {
            //@ts-ignore
        ...
        buildPath(filePath, extension) {
            ...
            if (moduleName === 'React' && this.reactReleaseMode && extension === 'js') {
        ...
        load(name) {
            ...
            try {
                ...
                if (loaderName === 'js' && exports) {
                    injectModuleName(exports, defineName, isObject, true);
