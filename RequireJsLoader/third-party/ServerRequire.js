...
(function () {
    ...
    if (typeof importScripts !== 'undefined') {
        load = (module) => {
            ...
            if (!module.defined && filesOfWithoutDdefine.includes(module.name)) {
                module.define([], () => null);
            }
        };
    }
    ...
    class ServerRequire extends BaseRequire {
        ...
        buildConfig({ root = '', resourcesRoot = '/', modules, pagexPackages, staticsDomain, sharedDomain = '', staticsRoot = '/resources/', metaRoot = '', cdnRoot = '/cdn/', contents, }) {
            ...
            for (const [name, moduleConfig] of Object.entries(modules)) {
                ...
                const rootPath = path ? path.slice(0, path.lastIndexOf('/') + 1) : resourcesRoot;
                ...
                const info = {
                    buildUrl(filePath, extension) {
                        ...
                        if (extension === 'css') {
                            const extensionCss = getDirection() === 'rtl' ? 'rtl.css' : 'css';
                            return `${domain}${rootUrl}${filePath}${postfixForMinVersion}.${extensionCss}${queryParams}`;
                        }
                        return `${domain}${rootUrl}${filePath}${postfixForMinVersion}.${extension}${queryParams}`;
                    },
                    buildPath(filePath, extension) {
                        return `${root}${rootPath}${filePath}.${extension}`;
                    },
                    hasTailwind,
                    templateExtension,
            ...
            this.modulesInfo.set('cdn', {
                ...
                buildPath(filePath, extension) {
                    ...
                    return `${root}/${filePath}.${extension}`;
                },
    ...
    // @ts-ignore
    globalEnv.initRequire = (root, resourcesRoot) => {
        var _a, _b, _c, _d, _e;
