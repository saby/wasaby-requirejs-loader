...
(function () {
    ...
    class BaseRequire {
        ...
        defineFeature(name, defaultDeps, defaultCallback) {
            ...
            const featureName = (_a = moduleInfo.features) === null || _a === void 0 ? void 0 : _a[name];
            if (!featureName) {
                return this.define(name, defaultDeps, defaultCallback);
            }
    ...
    function getFetchError(message, mode, start, httpCode, statusText) {
     ...
     * @param start
     */
    async function getResponse(url, mode, start) {
        try {
            ...
            if (err.name === 'TypeError') {
                throw getFetchError(`Network/CORS/browser error: ${err.message}`, mode, start);
            }
            throw getFetchError(`Unknown fetch error: ${err.message}`, mode, start);
        }
    }
    /**
    ...
    async function fetchLoader (url) {
        ...
        if (!response.ok) {
            if (response.status === NOT_FOUND_CODE) {
                throw getFetchError(`File not found.`, mode, start, response.status, response.statusText);
            }
    ...
    function enableBundles() {
        var _a;
        const cookie = ((_a = document.cookie.match(/(?:^|;)\s*disableBundles\s*=\s*([^;]+)/)) === null || _a === void 0 ? void 0 : _a[1]) || '';
    ...
    class WebRequire extends BaseRequire {
        constructor() {
            ...
            if (globalEnv.wsConfig.isDebugReact && this.debugCookies !== 'true') {
                if (this.debugCookies) {
                    this.debugCookies += ',React';
                }
                else {
                    this.debugCookies = 'React';
                }
    ...
    globalEnv.initRequire = () => {
        ...
        const hotReloadCookies = ((_a = document.cookie.match(/(?:^|;)\s*s3HotReload\s*=\s*([^;]+)/)) === null || _a === void 0 ? void 0 : _a[1]) || '';
