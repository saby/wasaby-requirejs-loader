...
(function () {
    ...
    class RequireError extends Error {
        ...
        static isRequireError(err) {
            ...
            return err?.requireError === true;
        }
    }

    // @ts-ignore
    const globalEnv$4 = globalThis;
    /**
     * Возвращает шардированый домен, если он есть
     */
    function getShardDomain() {
        return globalEnv$4.wsConfig.shardDomain || '';
    }

    const EMPTY_ARRAY$1 = [];
    function getAllStaticDomains() {
        // @ts-ignore
        const globalEnv = globalThis;
        if (Array.isArray(globalEnv.wsConfig.staticDomains)) {
            return globalEnv.wsConfig.staticDomains;
        }
        return globalEnv.wsConfig.staticDomains?.domains || EMPTY_ARRAY$1;
    }
    ...
    function getModuleResolution(reactVersion) {
        ...
        const map = {
            ...
            'pixi-react8': `${cdnName}PixiReact/8.0.4-p3/pixi-react.min.js`,
            // jQuery must die
    ...
    class BaseRequire {
        constructor() {
            const contents = globalEnv$3.contents || {};
            const wsConfig = globalEnv$3.wsConfig || {};
            this.modules = contents.modules || {};
            this.modulesResolution = getModuleResolution(this.modules.React?.version || DEFAULT_REACT_VERSION);
            ...
            this.metaRoot = wsConfig.metaRoot || globalEnv$3.metaRoot || '';
        ...
        getNumberStaticDomain() {
            return 0;
        }
        getCurrentStaticsDomain() {
            const domain = getAllStaticDomains()[this.getNumberStaticDomain()];
            return domain ? `//${domain}` : '';
        }
        buildDebugModules(debugCookie) {
        ...
        defineFeature(name, defaultDeps, defaultCallback) {
            ...
            if (featureName.includes(SLASH)) {
                return this.define(name, [featureName], (result) => result);
            }
            if (featureName === moduleName) {
                return this.define(name, defaultDeps, defaultCallback);
            }
            const featureFullName = name.replace(moduleName, featureName);
            return this.define(name, [featureFullName], (result) => result);
        }
    ...
    function isForwardRefOrMemo(value) {
        return (value !== null &&
            typeof value === 'object' &&
     ...
     */
    function markClientComponent(component, fullId, markClientReferences) {
     ...
     * @param markClientReferences Размечать клиентские ссылки (ServerRequire) или нет (WebRequire).
     */
    function markClientInWrapper(component, fullId, markClientReferences) {
        if (!markClientReferences) {
            return;
        }
     ...
     */
    function injectModuleName(obj, moduleName, isObject, markClientReferences = false) {
        ...
        typeof exports === 'object' &&
            ...
            Object.keys(exports).forEach((name) => {
                ...
                if (isForwardRefOrMemo(module)) {
                    ...
                    // как в rc-26.5000), а client-разметку ($$typeof/$$id) — только на сервере
                    // и только для не-".server"-модулей.
                    if (!module.hasOwnProperty('_moduleName')) {
                        module._moduleName = fullName;
                    }
    ...
    const disableLoadCss = !(globalEnv$2.wsConfig?.loadCss === undefined
        ? true
    ...
    function resolveSuffix(name) {
        return globalEnv$2.wsConfig?.themeName && name === IGNORE_MODULE;
    }
    /**
     ...
     */
    async function wml (defineName, filePath, context) {
        return js(defineName, `${filePath}.wml`, context);
    }
    ...
    function tmpl (defineName, filePath, context) {
        return js(defineName, `${filePath}.tmpl`, context);
    }
    ...
    async function json (defineName, filePath, context) {
        ...
        try {
            return JSON.parse(await context.loader.fetch(url));
        }
        catch (err) {
    ...
    async function text (defineName, filePath, context) {
        ...
        try {
            return await context.loader.fetch(url);
        }
        catch (err) {
    ...
    async function loadFullPackageInfo(UIModuleName, context) {
        ...
        // Если файлы, которые упакованы, были удалены, мы обязаны загрузить пакет, поэтому отключаем фильтрацию.
        if (context.packedFilesRemoved) {
            return {
                map: normalizeMap,
                disabledPackages,
                disabledMap: false,
    ...
    let actualContents;
    const scriptsWithHandlers = new WeakSet();
    ...
    function processNextResourcesBlock() {
        ...
        const callback = globalEnv$1.callbacksForResourcesBlock.get(nextResourcesBlock);
        if (!callback) {
            isProcessingResourcesBlock = false;
            return;
        }
        isProcessingResourcesBlock = true;
        globalEnv$1.callbacksForResourcesBlock.delete(nextResourcesBlock);
    ...
    function registryResourceInBlock(blockId) {
        const resourcesBlock = globalEnv$1.resourcesBlock?.get(blockId);
        if (!resourcesBlock) {
            return;
        }
        ...
        globalEnv$1.resourcesBlock?.delete(blockId);
        const requireListFn = () => {
        ...
        globalEnv$1.callbacksForResourcesBlock.set(blockId, requireListFn);
        processNextResourcesBlock();
    }
    function showModalForcedPageReload(message) {
  ...
  `;
    ...
    function checkFallbackCondition(domain) {
        const now = Date.now();
    ...
    function createTimeoutAbortSignal(timeout) {
    ...
    function createTimeoutAbortPromise(timeout, targetPromise) {
        return new Promise((resolve, reject) => {
    ...
    async function getActualContentsVersion() {
    ...
    async function loadActualContents() {
        const url = globalEnv$1.requirejs.instance.buildUrl('contents', 'json').replace('.min.', '.');
        const response = await fetch(url);
        if (response.ok) {
            return response.json();
        }
        return globalEnv$1.contents;
    }
    async function contentsIsActual() {
        if (Date.now() - lastCheckContents > TIME_OF_RELEVANCE_OF_CONTENTS) {
            ...
            if (globalEnv$1.contents?.buildnumber !== actualVersion) {
                actualContents = await loadActualContents();
                return false;
            }
            return true;
        }
        return globalEnv$1.contents?.buildnumber === actualContents.buildnumber;
    }
    async function pingFileWithActualVersion(moduleUrl, timeout) {
        const url = new URL(moduleUrl, location.href);
        const req = globalEnv$1.requirejs.instance;
    ...
    async function pingUrl(url, timeout) {
        ...
        try {
            ...
            throw new FetchError(undefined, err);
        }
        if (!response.ok) {
            throw new FetchError(response);
        }
    }
    async function convertToFetchError(error, url, timeout) {
    ...
    async function loadWithFallback(link, timeout, loader, error) {
        ...
        if (currentNumber === -1 || location.host === url.host) {
            ...
            if (err.code === NOT_FOUND_CODE) {
                ...
                globalEnv$1.requirejs.instance.downloadBlocked =
                    'Due to an outdated contents.js, requests for static content with 404 response codes were recorded.';
                showModalForcedPageReload('Из‑за устаревания страницы возникла проблема. Перезагрузите страницу для корректной работы.');
            }
            throw err;
        }
        const numCdnDomain = currentNumber + 1;
        const isCdnActual = globalEnv$1.requirejs.instance.currentNumberDomain === currentNumber;
        if (isCdnActual && checkFallbackCondition(url.host)) {
            ...
            globalEnv$1.requirejs.instance.currentNumberDomain = numCdnDomain;
        }
        url.host = staticDomains[numCdnDomain] || location.host;
        //@ts-ignore
        const dataset = error.target?.dataset;
    ...
    function tagScript(url, timeout, customAttr) {
        const fullUrl = new URL(url, location.href).href;
        const cachePromise = loadingUrls.get(fullUrl);
        if (cachePromise) {
            return cachePromise;
        ...
        return createTimeoutAbortPromise(timeout, loadPromise)
            ...
            .then(() => {
            const error = errorUrls.get(fullUrl);
            if (error) {
    ...
    async function fetchLoader(url, timeout) {
        const mode = isCrossOriginUrl(url) ? 'cors' : 'no-cors';
        const { signal } = createTimeoutAbortSignal(timeout);
        let response;
        try {
            ...
            return loadWithFallback(url, timeout, fetchLoader, new FetchError(undefined, err));
        }
        if (!response.ok) {
            return loadWithFallback(url, timeout, fetchLoader, new FetchError(response));
        }
        return response.text();
    }
    ...
    class Loader {
        constructor(timeout) {
            ...
            actualContents = globalEnv$1.contents;
            window.removeEventListener('error', globalEnv$1.onErrorRequireRegistry, true);
            window.addEventListener('error', (event) => this.processError(event), true);
            const callbacksForResourcesBlock = globalEnv$1.callbacksForResourcesBlock || new Map();
            ...
            //@ts-ignore
            globalEnv$1.callbacksForResourcesBlock = {
                set(key, value) {
            ...
            if (globalEnv$1.loadedScriptsFromBlock) {
                ...
                globalEnv$1.ols = (script) => {
                    if (script.dataset.rid) {
                        registryResourceInBlock(Number(script.dataset.rid));
                    }
                };
                for (const blocId of globalEnv$1.loadedScriptsFromBlock) {
                    registryResourceInBlock(blocId);
                }
                globalEnv$1.loadedScriptsFromBlock = undefined;
            }
            if (globalEnv$1.preRegistryErrors) {
                for (const error of globalEnv$1.preRegistryErrors) {
                ...
                globalEnv$1.preRegistryErrors.clear();
            }
        }
        processError(event) {
    ...
    const LIFETIME_CACHE_CDN_COOKIE = LIFETIME_FALLBACK_COOKIE * ONE_SECOND_IN_MS;
    const loaders = {
        wml,
        js,
        tmpl,
    ...
    function getCDNCookie() {
        return Number(document.cookie.match(/(?:^|;)\s*res_loader_cdn_idx\s*=\s*([^;]+)/)?.[1] || 0);
    }
    class WebRequire extends BaseRequire {
        constructor() {
            super();
            // TODO Нужно здесь, потомоу что Loader в констуркторе при обрботке зареганных ошибок смотрит в эту переменую.
            //  Быстрый фикс, но по надо подумать как сделать лушче
            // @ts-ignore
            globalEnv.requirejs = globalEnv.require = this.require.bind(this);
            // @ts-ignore
            globalEnv.requirejs.instance = this;
            ...
            this.debugModules = this.buildDebugModules(document.cookie.match(/(?:^|;)\s*s3debug\s*=\s*([^;]+)/)?.[1] || '');
            this.currentNumberDomain = getCDNCookie();
            lastCheckCookieCDN = Date.now();
            if (globalEnv.wsConfig.reactMode === 'debug') {
            ...
            this.cache.set('require', globalEnv.requirejs);
        ...
        getNumberStaticDomain() {
            const now = Date.now();
            // Раз в час нужно перевычиать куку, потому она живёт один час,
            // а значит она могла истечь и мы должны вернутся на основной домен.
            if (now - lastCheckCookieCDN > LIFETIME_CACHE_CDN_COOKIE) {
                const cdnNum = getCDNCookie();
                this.currentNumberDomain = cdnNum;
                lastCheckCookieCDN = now;
                return cdnNum;
            }
        ...
        loadModule(defineName, loaderName, modulePath) {
            ...
            if (!loadPromise) {
                ...
                if (this.enableBundles && moduleInfo && moduleInfo.hasBundles) {
                    loadPromise = bundleLoader(defineName, modulePath, loaderName, this, loaders[loaderName]);
                }
                else {
    ...
    globalEnv.initRequire = () => {
        ...
        const hotReloadCookies = document.cookie.match(/(?:^|;)\s*s3HotReload\s*=\s*([^;]+)/)?.[1] || '';
