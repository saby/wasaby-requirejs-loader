...
export interface IPatchedGlobal {
    ...
    preRequiredFeatures?: IPredefinedModules;

    preRegistryErrors?: Set<ErrorEvent>;
    onErrorRequireRegistry(event: ErrorEvent): void;
    onErrorHandler(event: ErrorEvent): void;

    loadedScriptsFromBlock?: number[];
    onloadScript?: (script: HTMLScriptElement) => void;
    ols?: (script: HTMLScriptElement) => void;
    registryOnLoadScriptFromBlock?: (blockId: number) => void;
    resourcesBlock?: Map<number, { count: number; rootModules: string[] }>;
    callbacksForResourcesBlock: Map<number, () => Promise<unknown>>;
}
...
export interface IWsConfig {
    ...
    reactMode?: 'release' | 'debug' | 'default';
}
