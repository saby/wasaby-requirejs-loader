...
function buildUrl(url: string): string {
    ...
    const info = loader.parseName(
        url.startsWith(resourceRoot) ? url.replace(resourceRoot, '') : url
    );
    const extension = info.filePath.split('.').pop();
    const modulesInfo = loader.modulesInfo.get(info.rootDir);

    if (modulesInfo) {
        if (info.rootDir === 'cdn') {
            return modulesInfo.buildUrl(info.filePath.replace(`.${extension}`, ''), extension);
        }

        // TODO совместимость под get_package у спецификаций, там суют ссылку уже с min расширением.
        const normName = info.filePath.replace('.min.', '.').replace(`.${extension}`, '');

        return modulesInfo.buildUrl(normName, extension);
    }

    if (META_NAME.has(info.defineName)) {
        return loader.modulesInfo
            .get('$default$')
            .buildUrl(info.filePath.replace(`.${extension}`, ''), extension);
    }
