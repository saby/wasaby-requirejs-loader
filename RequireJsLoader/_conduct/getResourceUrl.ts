...
function buildUrl(url: string): string {
    ...
    if (loader.modules.hasOwnProperty(rootDir)) {
        ...
        if (['tmpl', 'wml'].includes(extension)) {
            return loader.buildUrl(`${normName}.${extension}`, 'js');
        }
...
export default function getResourceUrl(
    ...
    // Специльно убрал из статичной зависимости чтобы он не грузился, старый уйгшку остался только в юнитах,
    // там он всегда предзагружен, поэтому можно и синхроно запросить.
    const { handlers } = requirejs('RequireJsLoader/config');
