...
export default class ModulesManager implements IModulesManager, IModulesManagerSync, IModulesHandler {
    ...
    loadModule<T>(name: string): Promise<T> {
        if (this.cache.loaded[name]) {
            return Promise.resolve(this.cache.loaded[name]);
        }

        if (!this.cache.loading[name]) {
            this.cache.loading[name] = new Promise((resolve, reject) => {

                this._loader([name], (...module) => {
                    this.cache.loaded[name] = module[0];
                    resolve(module[0]);
                }, (err) => {
        ...
        return this.cache.loading[name];
    }
    ...
    loadSync<T>(module: string): T {
        ...
        if (!this.cache.loaded[moduleName]) {
            this.cache.loaded[moduleName] = this._loader(moduleName);
        }

        return this.cache.loaded[moduleName];
    }
    ...
    unloadSync(module: string): void {
        ...
        delete this.cache.loaded[module];
        delete this.cache.loading[module];
