...
const META_NAME = new Set(['contents', 'router']);
...
function getModuleInfo(module: string, direction?: string): IModuleInfo {
    ...
    const isRootModule = basenameParts.length === 1 && META_NAME.has(basename);
...
export default function getModuleUrl(
    ...
    if (globalEnv.requirejs.isNewRequire) {
        // TODO Нам могут снуть готовый или полуготовый url, так быть не должно,
        //  но для временой соместимости будет возращать из как есть.
        //  В 26.1000 после внедрения везде нового require, надо будет сносить это и идти испралять тех кто это творит.
        if (module.includes('//') || module.includes('?x_module') || module.includes('#')) {
            return module;
        }

        const req = globalEnv.requirejs.instance;
        const defineName = req.getDefineName(module);
        const filePath = req.getModulePath(defineName);
        const loaderName = req.getLoaderName(defineName);
        const extension = module.split('.').pop();

        if (COMMON_EXTENSIONS.includes(extension as string)) {
            return req.buildUrl(filePath.replace(`.${extension}`, ''), extension);
        }

        // TODO надо будет удалить этот костыль, после того, как переведем все зависимости на .wml/.tmpl
        // вместо плагина wml!/tmpl!
        if (['tmpl', 'wml'].includes(loaderName)) {
            return req.buildUrl(`${filePath}.${loaderName}`, 'js');
        }

        return req.buildUrl(filePath, loaderName);
    }

    const info = getModuleInfo(module, direction);
    const config = globalEnv.wsConfig || {};
