...
export default function getModuleUrl(
    ...
    if (globalEnv.requirejs.isNewRequire) {
        ...
        const infoFile = req.parseName(module);
        const extension = module.split('.').pop();
        const modulesInfo =
            req.modulesInfo.get(infoFile.rootDir) || req.modulesInfo.get('$default$');

        if (COMMON_EXTENSIONS.includes(extension as string)) {
            return modulesInfo.buildUrl(infoFile.filePath.replace(`.${extension}`, ''), extension);
        }

        return modulesInfo.buildUrl(infoFile.filePath, infoFile.extension);
    }
