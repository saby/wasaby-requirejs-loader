...
import type { IPackagesInfo } from '../../main/ModuleInfo';
...
interface IModuleInfoWithPackage extends ModuleInfo {
...
function callLoader(
    ...
    if (packagesInfo.map.hasOwnProperty(module.name)) {
        ...
        const packageInfo = context.modulesInfo.get(packagePath.split('/')[0]) as ModuleInfo;
 ...
 * @param moduleInfo Информация о UI модулей, в котором живём модуль.
 */
export function hasPackages(moduleInfo: ModuleInfo): moduleInfo is IModuleInfoWithPackage {
    return moduleInfo.hasOwnProperty('packagesInfo') && !!moduleInfo.packagesInfo?.mapPath;
}
