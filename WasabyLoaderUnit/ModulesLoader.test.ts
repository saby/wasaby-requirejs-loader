...
const getPostponedModules = ModulesLoader.getPostponedModules;
const unloadSync = ModulesLoader.unloadSync;
...
describe('WasabyLoader/ModulesLoader', () => {
    ...
    describe('loadAsync()', () => {
        ...
        it('should load postponed module', (done) => {
            // чтобы проверить работу отложенного загрузчика, надо сначала выгрузить модуль из кеша
            // requirejs и wasabyloader, если он был ранее загружен.
            unloadSync('WasabyLoaderUnit/resources/TestLibraryAsync');
            loadAsync('WasabyLoaderUnit/resources/TestLibraryAsync', 3000).then((res) => {
                assert.notEqual(res, undefined, 'Module not loaded async');

                // после отработки таймаута и успешной загрузки модуля список postponed-модулей должен быть пустым
                assert.deepEqual(getPostponedModules(), []);
                done();
            });

            // после вызова loadAsync с таймаутом загружаемый модуль должен быть доступен в списке postponed-модулей
            // до момента полной загрузки запрашиваемого модуля.
            assert.deepEqual(getPostponedModules(), ['WasabyLoaderUnit/resources/TestLibraryAsync']);
        });
